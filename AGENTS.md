...
6. The local Vite showcase runs on `http://localhost:5173`. It is a black, high-contrast, local-only marketing prototype: it uses locally bundled Space Grotesk and DM Sans, and compact CSS glass navigation controls. Do not publish or deploy it without Justin explicitly asking.
