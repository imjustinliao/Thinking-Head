...
6. The local Vite showcase runs on `http://localhost:5173`. It is a black, high-contrast, local-only marketing prototype: it uses locally bundled Space Grotesk and DM Sans, plus one coherent glass material across navigation and the state rail. Material hierarchy, contrast, typography, motion, reduced motion, and reduced transparency follow the Apple Human Interface Guidelines. Do not publish or deploy it without Justin explicitly asking.
